...
class ZeroScope(nn.Module):
    ...
    def train_step(
        ...
        #     )
        grad, latent_prev = self.compute_grad_hifa(latents, text_embs, t, use_csd=True)
        grad = grad.to(latents.dtype)
        # grad = self.compute_grad_sds(latents, text_embs, t).to(latents.dtype)
        ...
        rgb_target = self.decode_latents(latent_prev).permute(0, 2, 1, 3, 4)
        # print(latents.dtype, target.dtype, pred_rgb_512.dtype)
        # print(latents.dtype, target.dtype, pred_rgb_512.dtype, rgb_target.dtype)
        # print(pred_rgb_512.min(), pred_rgb_512.max(), rgb_target.min(), rgb_target.max())
        # print(pred_rgb_512.shape, rgb_target.shape)
        loss2 = 0.05 * F.mse_loss(pred_rgb_512.float(), rgb_target.detach().float(), reduction="sum") / rgb_target.shape[0]
        # loss2 = 512 * F.mse_loss(pred_rgb_512, rgb_target.detach(), reduction='mean') / rgb_target.shape[0]
    ...
    def compute_grad_hifa(
        ...
        return grad, pred_original_sample

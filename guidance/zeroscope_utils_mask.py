...
class ZeroScope(nn.Module):
    def __init__(
        ...
        fp16=False,
        vram_O=False,
        t_range=[0.02, 0.98], ### why not using full range?
    ):
        # # sd_version="2.1",
        # hf_key=None,
        super().__init__()
    ...
    def train_step(
        ...
        mask_rgbs = mask_rgbs.to(self.dtype)
        if as_latent:
            latents = F.interpolate(pred_rgb, (40, 72), mode="bilinear", align_corners=False).permute(1, 0, 2, 3)[None]# * 2 - 1
            mask_rgbs = F.interpolate(mask_rgbs, (40, 72), mode="bilinear", align_corners=False).permute(1, 0, 2, 3)[None]# * 2 - 1
        else:
        ...
        #     )
        grad = self.compute_grad_sds(latents, text_embs, t, mask_rgbs)
        ...
        target = (latents*mask_rgbs - grad).detach()
        loss = 0.5 * F.mse_loss((latents*mask_rgbs).float(), target, reduction='sum') / latents.shape[0]
    ...
    def compute_grad_sds(
        ...
        #     )
        grad = w * (noise_pred - noise) * mask_rgbs
        return grad
...
if __name__ == "__main__":
    ...
    # plt.show()
    embeddings_set = []
    embs = zs.encode_text(['one flower and one bee, the bee is flying around the flower'])
    neg_prompt = zs.encode_text(["fast motion, text, watermark, copyright, blurry, nsfw"])
    # print(embs.shape, neg_prompt.shape)
    embeddings_1 = torch.cat([embs, neg_prompt], dim=0)  #
    embeddings_set.append(embeddings_1)

    embs = zs.encode_text(['grass'])
    neg_prompt = zs.encode_text(["flower, bee, fast motion, text, watermark, copyright, blurry, nsfw"])
    # print(embs.shape, neg_prompt.shape)
    embeddings_2 = torch.cat([embs, neg_prompt], dim=0)  #
    embeddings_set.append(embeddings_2)

    #   use_rgb = True
    use_rgb = False
    if use_rgb:
        rgbs = torch.rand(1, 3, 320, 576).cuda().repeat(16, 1, 1, 1)
        rgbs.requires_grad = True
        optimizer = torch.optim.Adam([rgbs], lr=0.05)
        for step in tqdm.tqdm(range(1000)):
            optimizer.zero_grad()
            loss_sds = zs.train_step(rgbs, embeddings_set)
            loss_sds.backward()
            optimizer.step()
            if step % 100 == 0 and step > 0:
                video_path = os.path.join('./output_rgb', f"sds_rgb_{step}.gif")
                save_results(rgbs.data.cpu().unsqueeze(0), video_path, fps=10)
    else:
        rgbs = torch.randn(1, 4, 40, 72).cuda().repeat(16, 1, 1, 1)
        mask_rgbs = torch.zeros(1, 4, 40, 72)
        mask_rgbs[:,:,10:30,20:60] = 1
        mask_rgbs = mask_rgbs.cuda().repeat(16, 1, 1, 1)
        rgbs.requires_grad = True
        optimizer = torch.optim.Adam([rgbs], lr=0.05)
        for step in tqdm.tqdm(range(10000)):
            optimizer.zero_grad()

            loss_sds = zs.train_step(rgbs, embeddings_set[0], mask_rgbs, as_latent=True) + zs.train_step(rgbs, embeddings_set[1], 1-mask_rgbs, as_latent=True)
            loss_sds.backward()
            if step % 20 == 0:
            ...
            with torch.no_grad():
                if step % 100 == 0:
                # if step % 100 == 0 and step > 0:
                    video_path = os.path.join('./output_t1', f"sds_{step}.gif")

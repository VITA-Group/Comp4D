...
class StableDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def get_objects_text_embeds(self, object_prompts, negative_prompts):
        for prompt in object_prompts:
            pos_embeds = self.encode_text([prompt])  # [1, 77, 768]
            neg_embeds = self.encode_text(negative_prompts)
            self.object_embeddings.append(torch.cat([neg_embeds, pos_embeds], dim=0))  # [2, 77, 768]
    ...
    def train_step(
        ...
        with torch.no_grad():
            ...
            if obj_id is not None:
                noise_pred = self.unet(
                    latent_model_input, tt, encoder_hidden_states=self.object_embeddings[obj_id].repeat(batch_size, 1, 1)
                ).sample
            else:
                noise_pred = self.unet(

...
from attn_utils.unet_attn import UNet3DConditionModel_Attn
from attn_utils.attention_refocusing_loss import caculate_loss_att_fixed_cnt, caculate_loss_self_att
class ZeroScope(nn.Module):
    def __init__(
        ...
        # self.unet = pipe.unet
        # self.unet = UNet3DConditionModel_Attn.from_config(pipe.unet.config).to(device)
        self.unet = UNet3DConditionModel_Attn.from_pretrained(model_key, subfolder='unet', torch_dtype=self.dtype).to(device)
        self.unet.eval()
    ...
    @torch.no_grad()
    def get_text_embeds(self, prompts, negative_prompts=['low motion, static statue, not moving, no motion, text, watermark, copyright, blurry, nsfw']):
    ...
    def forward_unet(
        ...
        res, t2d_attn_weight_listss, t2d_cross_attn_weight_listss, temp_attn_weight_listss, temp_cross_attn_weight_listss = self.unet(
            latents.to(self.weights_dtype),
            t.to(self.weights_dtype),
            encoder_hidden_states=encoder_hidden_states.to(self.weights_dtype),
        )
        res = res.sample.to(input_dtype)
        return res, t2d_attn_weight_listss, t2d_cross_attn_weight_listss, temp_attn_weight_listss, temp_cross_attn_weight_listss
    ...
    def refine(self, pred_rgb,
        ...
        # latents = torch.randn((1, 4, 48, 48), device=self.device, dtype=self.dtype)
    ...
    def train_step(
        ...
        if as_latent:
            latents = F.interpolate(pred_rgb, (48, 48), mode="bilinear", align_corners=False).permute(1, 0, 2, 3)[None]# * 2 - 1
        else:
            # interp to 512x512 to be fed into vae.
            pred_rgb_512 = F.interpolate(pred_rgb, (320, 576), mode="bilinear", align_corners=False).permute(1, 0, 2, 3)[None]
            # encode image into latents with vae, requires grad!
            latents = self.encode_images(pred_rgb_512) ### doublecheck
        # print(latents.shape)
        ...
        #     )
        grad, t2d_attn_weight_listss, t2d_cross_attn_weight_listss, temp_attn_weight_listss, temp_cross_attn_weight_listss = self.compute_grad_sds(latents, text_embs, t)
        ...
        # for i, list in enumerate(t2d_attn_weight_listss):
        #     print(i, len(list))
        #     for j, attn in enumerate(list):
        #         print(j, type(attn), attn.shape)

        # for i, list in enumerate(t2d_cross_attn_weight_listss):
        #     print(i, len(list))
        #     for j, attn in enumerate(list):
        #         print(j, type(attn), attn.shape)

        bboxes = [np.array([[0.01953125, 0.08984375, 0.40039062, 0.52929688]]), np.array([[0.45898438, 0.20117188, 0.99804688, 0.79882812]])]
        object_positions = [[2], [10]]
        loss1 = caculate_loss_self_att(t2d_attn_weight_listss, bboxes=bboxes)*self.opt.w_l1
        loss2, min_inside, max_outside = caculate_loss_att_fixed_cnt(t2d_cross_attn_weight_listss, bboxes=bboxes,
                            object_positions=object_positions, t = 0)
        loss2= loss2 * self.opt.w_l2
    ...
    def compute_grad_sds(
        ...
        noise_pred, t2d_attn_weight_listss, t2d_cross_attn_weight_listss, temp_attn_weight_listss, temp_cross_attn_weight_listss = self.forward_unet(
                latent_model_input,
                torch.cat([t] * 2),
        ...
        grad = w * (noise_pred - noise)
        return grad, t2d_attn_weight_listss, t2d_cross_attn_weight_listss, temp_attn_weight_listss, temp_cross_attn_weight_listss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, default='a bee is flying on the left of a flower')
    ...
    zs = ZeroScope(device, opt.fp16, opt.vram_O, opt=opt)
    ...
    # plt.imshow(imgs[0])
    # plt.show()
    embs = zs.encode_text([opt.prompt])
    ...
    if opt.use_rgb:
        rgbs = torch.randn(1, 3, 320, 576).cuda().repeat(16, 1, 1, 1).clamp(0, 1)
        ...
        for step in tqdm.tqdm(range(1001)):
            ...
            if step % 100 == 0 and step > 0:
                video_path = os.path.join(opt.save_dir, f"sds_rgb_{step}.gif")
                save_results(rgbs.data.cpu().unsqueeze(0), video_path, fps=10)
    else:
        rgbs = torch.randn(1, 4, 8, 8).cuda().repeat(16, 1, 1, 1)
        rgbs.requires_grad = True
        optimizer = torch.optim.Adam([rgbs], lr=0.1)
        for step in tqdm.tqdm(range(1001)):
            optimizer.zero_grad()
            ...
            with torch.no_grad():
                if step % 100 == 0:
                # if step % 100 == 0 and step > 0:
                    video_path = os.path.join(opt.save_dir, f"sds_{step}.gif")

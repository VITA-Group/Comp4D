...
from scene.deformation_nogrid import deform_network
...
class GaussianModel_nogrid:
    ...
    def load_colmap_ply(self, path, spatial_lr_scale=1, time_line=4):
        # https://github.com/graphdeco-inria/gaussian-splatting/blob/f11001b46c5c73a0a7d553353c898efd68412abe/scene/dataset_readers.py#L107
        plydata = PlyData.read(path)
        vertices = plydata['vertex']
        positions = np.vstack([vertices['y'], vertices['z'], vertices['x']]).T # [N, 3]
        # positions = positions[::2]
        print('Loaded points from ply ', positions.shape)
        colors = np.zeros_like(positions) + 0.5
        pcd = BasicPointCloud(points=positions, colors=colors, normals=None)
        self.create_from_pcd(pcd, spatial_lr_scale=spatial_lr_scale, time_line=time_line)

    def load_3studio_ply(self, path, spatial_lr_scale=1, time_line=4, pts_num=1, position_scale=1, load_color=True):
        # https://github.com/graphdeco-inria/gaussian-splatting/blob/f11001b46c5c73a0a7d553353c898efd68412abe/scene/dataset_readers.py#L107
        plydata = PlyData.read(path)
        vertices = plydata['vertex']
        positions = np.vstack([vertices['x'], vertices['z'], -vertices['y']]).T # [N, 3] # image dream axis
        # positions = np.vstack([vertices['y'], vertices['z'], vertices['x']]).T # [N, 3] # 3studio coord is this
        tot_num = positions.shape[0]
        new_idx = np.random.permutation(tot_num)[:pts_num]
        # positions = positions[::step] * position_scale
        positions = positions[new_idx] * position_scale
        print('Loaded points from ply ', positions.shape)
        # positions = np.vstack([vertices['x'], vertices['y'], vertices['z']]).T # [N, 3]
        # positions=np.concatenate([-positions[:,0:1],-positions[:,1:2],-positions[:,2:3]],1)#*train_dataset.scale_factor)
        if load_color:
    ...
    def random_init(self, num_pts, lr=10, radius=1):
        ...
        self.create_from_pcd(pcd, lr, 4) # 4 not used
    ...
    def training_setup(self, training_args):
        ...
        #                                             lr_delay_mult=training_args.deformation_lr_delay_mult,
        #                                             max_steps=training_args.position_lr_max_steps)    
        self.gs_neighbor_dist, self.gs_neighbor = self.get_k_nearest_neighbor()
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        print('split', padded_grad.mean(), grad_threshold)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        # Extract points that satisfy the gradient condition
        print('clone', torch.norm(grads, dim=-1).mean(), grad_threshold)
    ...
    def get_k_nearest_neighbor(self, k=40):
        bs = 1000
        index_list = []
        dist_list = []
        for i in range(0, self._xyz.shape[0], bs):
            x = self._xyz[i:i+bs]
            dists = torch.cdist(x, self._xyz)
            topdist, topk = torch.topk(dists, k + 1, dim=1, largest=False)
            topk = topk[:, 1:]
            topdist = topdist[:, 1:]
            
            index_list.append(topk)
            dist_list.append(topdist)
        
        index_list = torch.cat(index_list)
        dist_list = torch.cat(dist_list)
        print('neighbors', dist_list.shape, index_list.shape)
        return dist_list, index_list
    
    def get_nn_loss(self, dx):
        # idx = torch.randperm(40)
        neighbors = dx[self.gs_neighbor]
        # print('neighbors dx', neighbors.shape)
        return (dx.reshape(-1, 1, 3) - neighbors).pow(2).sum(-1).sum(-1).mean()

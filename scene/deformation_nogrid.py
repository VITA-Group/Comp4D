...
class Deformation(nn.Module):
    ...
    def create_net(self):
        ...
        mlp_out_dim = 0
        if self.no_grid:
            self.feature_out = [nn.Linear(32,self.W)]
        else:
        ...
        for i in range(self.D-1):
            if i % 2 == 0:
                self.feature_out.append(nn.LayerNorm(self.W))
    ...
    def query_time(self, rays_pts_emb, scales_emb, rotations_emb, time_emb):
        ...
        if self.no_grid:
            h = torch.cat([rays_pts_emb[:,:3],time_emb[:,:1] * 3],-1)
        else:
            raise NotImplementedError
        ...
        # print('input feature for grid', h.shape)
        h = self.pe(h)
        # print('position embedding', h.shape)
        h = self.feature_out(h)
        # print('embedded shared feature', h.shape)
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb,color_emb, time_emb):
        ...
        # dx = F.sigmoid(dx) * 2 - 1 # [make sure range is (-1, 1)]
        dx = F.tanh(dx / 0.5) * 0.2
        # dx = F.tanh(dx / 0.5) * 0.5
        # dx = dx * (time_emb ** 0.35)
        pts = rays_pts_emb[:, :3] + dx
        # print('deformation', rays_pts_emb[:, :3].shape, dx.shape)
        # print('dx', dx.min(), dx.max(), dx.mean())
...
class deform_network(nn.Module):
    def __init__(self, args) :
        ...
        # print(self)
        nn.init.constant_(self.deformation_net.pos_deform.fc1.weight, 0)
        nn.init.constant_(self.deformation_net.pos_deform.fc1.bias, 0)

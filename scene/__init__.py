...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None,shuffle=True, resolution_scales=[1.0], load_coarse=False):
        ...
        xyz_max = [2.5, 2.5, 2.5]
        xyz_min = [-2.5, -2.5, -2.5]
        if not self.gaussians._deformation.deformation_net.no_grid:
        ...
        if self.loaded_iter:
            ...
            # random init
            # self.gaussians.random_init(num_pts, 10, radius=0.5)
            # point cloud init
            
            # cloud_path='./data/eagle1_1.ply' # 
            # cloud_path = './data/crackling_campfire.ply'
            # cloud_path = './data/panda_dancing.ply'
            # cloud_path = './data/bee.ply'
